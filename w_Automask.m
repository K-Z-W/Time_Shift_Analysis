...
[Volume4D, VoxelSize, Header] = rp_readfile(Data);%Revised by Wei Wei 160601, change to use rp_readfile. 
                                                  %Revised by YAN Chao-Gan 140401, change to use Read All.
...
%     clip_fixed=w_ClipLevel(0.5, MeanAbsVolume);
    ...
    clip_fixed=w_ClipLevel(1, MeanAbsVolume);      % is dilate???
    
    fprintf('Fixed Clip level = %f\n', clip_fixed)
end

clip_mask=CliplevelGradual(0.5, MeanAbsVolume); %Get the Center of mass of a 3D image (in index coordinates)
...
if exist('OutputFile', 'var')==1 && ~isempty(OutputFile)
    ...
    rp_writefile(mask,OutputFile,size(mask),VoxelSize, Header,'double');%Revised by Wei Wei 160601, change to use rp_writefile to output the mask. 
                                                                        %Revised by YAN Chao-Gan 140401, Output the mask.
end
